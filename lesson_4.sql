...
-- РїРѕР»СЊР·РѕРІР°С‚РµР»Рё СЃРѕРѕР±С‰РµСЃС‚РІР°
...
-- РїСЂРѕС„РёР»СЊ РїРѕР»СЊР·РѕРІР°С‚РµР»СЏ
...
SELECT users.id, users.firstname, users.lastname, hometown, photo_id, filename FROM users LEFT JOIN profiles on id = user_id
left join media on media.id = photo_id; 
...
SELECT filename FROM media LEFT JOIN users on user_id = users.id
left join media_types as mt on mt.id = media_type_id
where email = 'arlo50@example.org' and mt.name_type = 'photo';
...
SELECT firstname, lastname FROM friend_requests AS fr left join users on users.id = target_user_id
WHERE initiator_user_id = 1 AND status = 'approved'
union 
SELECT firstname, lastname FROM friend_requests AS fr left join users on users.id = initiator_user_id
WHERE target_user_id = 1 AND status = 'approved';

-- Задача 6: Список медиафайлов пользователей, указав название типа медиа (id, filename, name_type)(используя JOIN)

SELECT lastname, firstname, name_type, filename FROM media LEFT JOIN users on user_id = users.id
left join media_types as mt on mt.id = media_type_id
order by lastname;


-- Задача 5: Список медиафайлов пользователей с количеством лайков (используя JOIN)
SELECT lastname, firstname, filename, ct_lk.countlikes FROM media LEFT JOIN users on user_id = users.id
left join media_types as mt on mt.id = media_type_id 
left join (SELECT media_id, count(id) as countlikes FROM likes group by media_id) as ct_lk on ct_lk.media_id = media.id
order by lastname;
